...
void	textnsw(char *line, int i, t_struct *g)
{
		...
		&& line[i + 1] == 'O' && (++(g->map_conf).counter))
	{
		(g->map_conf).north_texture
			= fill_textures((g->map_conf).north_texture, line, i);
		(g->map_conf).no += 200;
	}
		...
		&& line[i + 1] == 'O' && (++(g->map_conf).counter))
	{
		(g->map_conf).south_texture
			= fill_textures((g->map_conf).south_texture, line, i);
		(g->map_conf).so += 500;
	}
		...
		&& line[i + 1] == 'E' && (++(g->map_conf).counter))
	{
		(g->map_conf).west_texture
			= fill_textures((g->map_conf).west_texture, line, i);
		(g->map_conf).we += 40;
	}
}
...
void	textesf(char *line, int i, t_struct *g)
{
		...
		&& line[i + 1] == 'A' && (++(g->map_conf).counter))
	{
		(g->map_conf).east_texture
			= fill_textures((g->map_conf).east_texture, line, i);
		(g->map_conf).ea += 600;
	}
	else if (line[i] != '\0' && line[i] == 'S'
		&& line[i + 1] == ' ' && (++(g->map_conf).counter))
	{
		(g->map_conf).sprite
			= fill_textures((g->map_conf).sprite, line, i);
		(g->map_conf).s += 20;
	}
