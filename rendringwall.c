...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/04/27 15:40:56 by aaqari            #+#    #+#             */
...
void	initalize(t_r_struct *rays, int i, t_struct *g)
{
	...
	g->wall.projWallHeight = (TILE_SIZE / g->wall.perpDistance)
		* g->wall.distProjPlan;
	g->wall.wallStrHeight = (int)g->wall.projWallHeight;
	g->wall.wallTpPixel = (g->map_conf.height / 2)
		- (g->wall.wallStrHeight / 2);
	if (g->wall.wallTpPixel < 0)
		g->wall.wallTpPixel = 0;
	else
		g->wall.wallTpPixel = g->wall.wallTpPixel;
	g->wall.wallBtPixel = (g->map_conf.height / 2)
		+ (g->wall.wallStrHeight / 2);
